...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (bookingDone) {
    bookingDone.addEventListener('click', function() {
      ...
      if (selectedPartySize && selectedDate && selectedTime && bookingLabel) {
        ...
        bookingLabel.textContent = `${selectedPartySize.textContent} • ${selectedTime} ${formattedDate}`;
      }
      hideModal();
    });
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (loginSubmit) {
    loginSubmit.addEventListener('click', function() {
      ...
      if (name && phone && countryCode && name.value && phone.value) {
        console.log('Login attempt:', { name: name.value, phone: countryCode.value + phone.value });
...
// Restaurant booking function (for map popups)
function bookRestaurant(restaurantName) {
  ...
  if (bookingModal && modalOverlay) {
    ...
    if (modalTitle) {
      modalTitle.textContent = `Book ${restaurantName}`;
    }
  }
} 